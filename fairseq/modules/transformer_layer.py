...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None):
        ...
        # todo uncomment
        """<<< to use floating mask, this has to be deactivated
        """
        if attn_mask is not None:
            attn_mask = attn_mask.masked_fill(attn_mask.bool(), -1e8)

        # todo uncomment this to use numerical additive attention
        """
        if attn_mask is not None:
            attn_mask = attn_mask.masked_fill(attn_mask < 1e-6, -1e8)
        """
